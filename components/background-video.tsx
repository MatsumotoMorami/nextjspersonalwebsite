export function Background({ isMute, visible }: { isMute: boolean, visible: string }) {
    ...
    return (
        <div className="relative top-0 w-full h-screen overflow-hidden">
            ...
            {visible == "v" ? <div className="-z-10 fixed top-0 left-0 w-full h-full bg-black opacity-50" /> : undefined}
            {visible == "v" ? <div className="-z-10 fixed top-0 left-0 w-full h-full bg-white opacity-60" /> : undefined}
            <div className="-z-10 fixed top-0 left-0 w-full h-full" style={{
